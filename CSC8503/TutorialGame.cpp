	...
	if (inSelectionMode) {
		...
		if (Window::GetMouse()->ButtonDown(NCL::MouseButtons::Left)) {
			...
			if (world->Raycast(ray, closestCollision, true)) {
				...
				Debug::DrawLine(ray.GetPosition(), selectionObject->GetTransform().GetPosition(), Debug::GREEN, 3.0f);
				return true;
			}
			else {
				Debug::DrawLine(ray.GetPosition(), ray.GetPosition() + ray.GetDirection() * 100.0f, Debug::RED, 3.0f);
				return false;
			}
		}
